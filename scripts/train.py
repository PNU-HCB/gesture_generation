...
def train_epochs(args, train_data_loader, test_data_loader, lang_model, pose_dim, speaker_model=None):
    ...
    for epoch in range(args.epochs):
        ...
        for iter_idx, data in enumerate(train_data_loader, 0):
            global_iter += 1
            in_text, text_lengths, in_text_padded, _, target_vec, aux_info = data
            ...
            loss = []
            if args.model == 'multimodal_context':
                loss = train_iter_gan(args, epoch, in_text_padded, target_vec, vid_indices,
                                      generator, discriminator,
                                      gen_optimizer, dis_optimizer)
            elif args.model == 'joint_embedding':
                loss = train_iter_embed(args, epoch, in_text_padded, target_vec,
    ...
    with torch.no_grad():
        for iter_idx, data in enumerate(test_data_loader, 0):
            ...
            if args.model == 'joint_embedding':
                loss, out_dir_vec = eval_embed(in_text_padded, pre_seq_partial,
                                               target, generator, mode='speech')
            elif args.model == 'gesture_autoencoder':
                loss, _ = eval_embed(in_text_padded, pre_seq_partial, target, generator)
            elif args.model == 'seq2seq':
                ...
                out_dir_vec, *_ = generator(pre_seq, in_text_padded, vid_indices)
            ...
            if args.model != 'gesture_autoencoder':
                if embed_space_evaluator:
                    embed_space_evaluator.push_samples(in_text_padded, out_dir_vec, target)
...
def evaluate_sample_and_save_video(epoch, prefix, test_data_loader, generator, args, lang_model,
    ...
    with torch.no_grad():
        for iter_idx, data in enumerate(test_data_loader, 0):
            ...
            in_text, text_lengths, in_text_padded, _, target_dir_vec, aux_info = data
            ...
            if args.model == 'multimodal_context':
                ...
                _, _, _, _, _, _, out_dir_vec = generator(in_text_padded, pre_seq_partial, None, 'speech')
            elif args.model == 'gesture_autoencoder':
                _, _, _, _, _, _, out_dir_vec = generator(in_text_padded, pre_seq_partial, target_dir_vec,
            ...
            utils.train_utils.create_video_and_save(
                ...
                sentence, audio=None, aux_str=aux_str)
